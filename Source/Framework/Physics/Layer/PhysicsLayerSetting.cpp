...
FWK::Physics::PhysicsLayerSetting::PhysicsLayerSetting() : 
	m_broadPhaseLayerInterface(static_cast<JPH::uint>(Enum::PhysicsObjectLayerType::Count), static_cast<JPH::uint>(Enum::PhysicsObjectLayerType::Count)),
	m_objectLayerPairFilter   (static_cast<JPH::uint>(Enum::PhysicsObjectLayerType::Count))
{
	// 1. 先にObjectLayerをBroadPhaseLayerへ対応付ける
	SetupBroadPhaseLayerMapping();

	// 2. 次にObjectLayer同氏の衝突可否を設定する。
	SetupObjectLayerCollisionFilter();

	// 3.最後にObjectLayerVSBroadPhaseLayerのFilterを作る
...
{
	FWK_ASSERT_RETURN_VALUE_IF_FAILED(a_objectLayerType == Enum::PhysicsObjectLayerType::Invalid ||
...
{
	FWK_ASSERT_RETURN_IF_FAILED(a_layerA == Enum::PhysicsObjectLayerType::Invalid ||
								...
								"無効なPhysicsObjectLayerType同氏の衝突を有効化しようとしました");
