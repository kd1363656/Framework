...
namespace FWK::Graphics
{
	...
	public:
		...
		void SetupRoot32BitConstants(const RootConstantType& a_rootConstantData, const RootSignature& a_rootSignature, const Enum::RootParameterType a_rootParameterType)
		{
			// CommandListへByte列として記録するため
			// memcpy可能な単純データ型だけを許可する
			static_assert(std::is_trivially_copyable_v<RootConstantType>, "Root32BitConstantsへ渡す方は、triviallyCopyableである必要があります。");

			// RootConstantsは32bit単位で設定する。
			// そのため、構造体サイズが48Bte単位で
			// 割り切れなければ使用できない。
