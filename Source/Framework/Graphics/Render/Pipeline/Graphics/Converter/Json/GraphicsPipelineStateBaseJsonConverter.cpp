...
void FWK::Converter::GraphicsPipelineStateBaseJsonConverter::Deserialize(const nlohmann::json& a_rootJson, Graphics::GraphicsPipelineStateBase& a_graphicsPipelineStateBase) const
{
	...
	if (const auto& l_json = a_rootJson.value(k_rasterizerDescJsonKey, nlohmann::json{});
		!l_json.is_null())
	{
		DeserializeRasterizerDesc(l_json, a_graphicsPipelineStateBase);
	}
	...
	if (const auto& l_json = a_rootJson.value(k_blendDescJsonKey, nlohmann::json{});
		!l_json.is_null())
	{
		DeserializeBlendDesc(l_json, a_graphicsPipelineStateBase);
	}
	...
	if (const auto& l_json = a_rootJson.value(k_depthStencilDescJsonKey, nlohmann::json{});
		!l_json.is_null())
	{
		DeserializeDepthStencilDesc(l_json, a_graphicsPipelineStateBase);
	}
	...
	if (const auto& l_json = a_rootJson.value(k_rtvFormatListJsonKey, nlohmann::json{});
		!l_json.is_null())
	{
		DeserializeRTVFormatList(l_json, a_graphicsPipelineStateBase);
	}
	...
	a_graphicsPipelineStateBase.SetSampleMask		    (l_sampleMask);
}
...
nlohmann::json FWK::Converter::GraphicsPipelineStateBaseJsonConverter::Serialize(const Graphics::GraphicsPipelineStateBase & a_graphicsPipelineStateBase) const
{
	...
	l_rootJson[k_rasterizerDescJsonKey] = SerializeRasterizerDesc(a_graphicsPipelineStateBase);

	// ブレンドデスクのシリアライズ
	l_rootJson[k_blendDescJsonKey] = SerializeBlendDesc(a_graphicsPipelineStateBase);

	// 深度バッファデスクのシリアライズ
	l_rootJson[k_depthStencilDescJsonKey] = SerializeDepthStencilDesc(a_graphicsPipelineStateBase);

	// RTVフォーマットリストのシリアライズ
	l_rootJson[k_rtvFormatListJsonKey] = SerializeRTVFormatList(a_graphicsPipelineStateBase);

	// DSVフォーマトのシリアライズ
	l_rootJson[k_dsvFormatJsonKey] = a_graphicsPipelineStateBase.GetVALDSVFormat();

	// サンプルデスクのシリアライズ
	l_rootJson[k_sampleDescJsonKey] = SerializeSampleDesc(a_graphicsPipelineStateBase);

	// 使用するルートシグネチャタイプのをシリアライズ
	l_rootJson[k_primitiveTopologyTypeJsonKey] = a_graphicsPipelineStateBase.GetVALPrimitiveTopologyType();
	l_rootJson[k_sampleMaskJsonKey]            = a_graphicsPipelineStateBase.GetVALSampleMask           ();
...
void FWK::Converter::GraphicsPipelineStateBaseJsonConverter::DeserializeRasterizerDesc(const nlohmann::json& a_rootJson, Graphics::GraphicsPipelineStateBase& a_graphicsPipelineStateBase) const
{
	...
	a_graphicsPipelineStateBase.SetRasterizerDesc(l_rasterizerDesc);
}
...
void FWK::Converter::GraphicsPipelineStateBaseJsonConverter::DeserializeDepthStencilDesc(const nlohmann::json & a_rootJson, Graphics::GraphicsPipelineStateBase & a_graphicsPipelineStateBase) const
{
	...
	a_graphicsPipelineStateBase.SetDepthStencilDesc(l_depthStencilDesc);
}
void FWK::Converter::GraphicsPipelineStateBaseJsonConverter::DeserializeRTVFormatList(const nlohmann::json & a_rootJson, Graphics::GraphicsPipelineStateBase & a_graphicsPipelineStateBase) const
{
	...
	{
		a_graphicsPipelineStateBase.AddRTVFormat(l_json.get<DXGI_FORMAT>());
	}
}
void FWK::Converter::GraphicsPipelineStateBaseJsonConverter::DeserializeSampleDesc(const nlohmann::json & a_rootJson, Graphics::GraphicsPipelineStateBase & a_graphicsPipelineStateBase) const
{
	...
	l_sampleDesc.Count = a_rootJson.value(k_countJsonKey, Converter::SwapChainJsonConverter::k_defaultSampleCount);

	// サンプル品質レベル通常の既定値は0。
	l_sampleDesc.Quality = a_rootJson.value(k_qualityJsonKey, Converter::SwapChainJsonConverter::k_defaultSampleQuality);
...
{
		  nlohmann::json l_rootJson	      = {};
	const auto&          l_rasterizerDesc = a_graphicsPipelineStateBase.GetREFRasterizerDesc();
...
{
	      nlohmann::json l_rootJson  = {};
	const auto&			 l_blendDesc = a_graphicsPipelineStateBase.GetREFBlendDesc();
...
{
		  nlohmann::json l_rootJson	        = {};
	const auto&			 l_depthStencilDesc = a_graphicsPipelineStateBase.GetREFDepthStencilDesc();
...
nlohmann::json FWK::Converter::GraphicsPipelineStateBaseJsonConverter::SerializeRTVFormatList(const Graphics::GraphicsPipelineStateBase& a_graphicsPipelineStateBase) const
{
	...
	const auto& l_rtvFormatList = a_graphicsPipelineStateBase.GetREFRTVFormatList();
...
{
	nlohmann::json l_rootJson   = {};
	const auto&	   l_sampleDesc = a_graphicsPipelineStateBase.GetREFSampleDesc();
