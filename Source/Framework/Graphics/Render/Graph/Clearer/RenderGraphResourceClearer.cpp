...
bool FWK::Graphics::RenderGraphResourceClearer::ClearRenderTargetPassTexture(const ResourceContext& a_resourceContext, const Renderer& a_renderer, const RenderTargetPassTexture& a_renderTargetPassTexture) const
{
	...
	FWK_ASSERT_RETURN_VALUE_IF_FAILED(l_renderTargetTexture.GetVALCurrentResourceState() != D3D12_RESOURCE_STATE_RENDER_TARGET, "RenderTargetPassTextureがD3D12_RESOURCE_STATE_RENDER_TARGETではない状態でClearしようとしています。beforUsage/afterUsageの指定、またはRenderGraphの自動遷移を確認してください。", false);
...
bool FWK::Graphics::RenderGraphResourceClearer::ClearDepthStencilPassTexture(const ResourceContext& a_resourceContext, const Renderer& a_renderer, const DepthStencilPassTexture& a_depthStencilPassTexture) const
{
	...
	FWK_ASSERT_RETURN_VALUE_IF_FAILED(!l_gpuResource.m_resource,																  "DepthStencilPassTextureのGPUResourceが無効のため、Clear処理に失敗しました。",    false);
	FWK_ASSERT_RETURN_VALUE_IF_FAILED(l_depthStencilTexture.GetVALDSVDescriptorIndex() == Constant::k_invalidDescriptorIndex, "DepthStencilPassTextureのDSVDescriptorIndexが無効のため、Clearに失敗しました。", false);

	// RenderGraph側でリソースの遷移漏れがあればassert
	FWK_ASSERT_RETURN_VALUE_IF_FAILED(l_depthStencilTexture.GetVALCurrentResourceState() != D3D12_RESOURCE_STATE_DEPTH_WRITE, "DepthStencilPassTextureがD3D12_RESOURCE_STATE_DEPTH_WRITEではない状態でClearしようとしています。beforUsage/afterUsageの指定、またはRenderGraphの自動遷移を確認してください。", false);
