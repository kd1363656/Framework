...
FWK::TypeAlias::DescriptorIndex FWK::Graphics::DrawRequestPerObjectBase::FetchTextureSRVDescriptorIndex(const std::weak_ptr<TextureRecord>& a_textureRecord) const
{
	...
	FWK_ASSERT_RETURN_VALUE_IF(!l_textureRecord, "TextureRecordが無効なため、TextureSRVDescriptorIndexの取得に失敗しました。", Constant::k_invalidDescriptorIndex);
...
{
	FWK_ASSERT_RETURN_VALUE_IF(!a_texture, "Textureが無効なため、TextureSRVDescriptorIndexの取得に失敗しました。", Constant::k_invalidDescriptorIndex);
