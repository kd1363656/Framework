...
{
    // 前フレームのSprite描画申請を消す
    m_drawRequestDataSmartPointerVectorList.RemoveExpiredElements();
}
...
void FWK::Graphics::SpriteScreenPerObjectDrawRequest::SetupPerObjectConstantBuffer(const Renderer& a_renderer, const RootSignature& a_rootSignature, const FrameResource& a_frameResource)
{
    ...
    const auto& l_drawRequestDataList = m_drawRequestDataSmartPointerVectorList.GetREFElementDataList();

    for (const auto& l_drawRequestData : l_drawRequestDataList)
    {
