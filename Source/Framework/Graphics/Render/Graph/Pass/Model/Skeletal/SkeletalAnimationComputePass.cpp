...
void FWK::Graphics::SkeletalAnimationComputePass::Execute(Renderer& a_renderer, RenderGraph& a_renderGraph)
{
	...
	const auto& l_localMatrixRootSignature = SetupComputeRenderPipeline(a_renderer, Enum::PipelineStateType::SkeletalAnimationLocalMatrix).lock();

	FWK_ASSERT_RETURN_IF(!l_localMatrixRootSignature, "LocalMatrix計算用RootSignatureを取得できないため、SkeletalAnimationComputePassの実行に失敗しました。");

	// 登録されている各SkeletalAnimationPlayerについて、
	// KeyFrameから各BoneのLocalMatrixを計算する
	l_skeletalAnimationPerObjectComputeRequest->DispatchLocalMatrix(a_renderer, *l_localMatrixRootSignature, *l_currentFrameResource);

	// LocalMatrixへ親BoneのGlobalMatrixを反映する
	const auto& l_boneHierarchyRootSignature = SetupComputeRenderPipeline(a_renderer, Enum::PipelineStateType::SkeletalAnimationBoneHierarchy).lock();

	FWK_ASSERT_RETURN_IF(!l_boneHierarchyRootSignature, "BoneHierarchy計算用RootSignatureを取得できないため、SkeletalAnimationComputePassの実行に失敗しました。");

	l_skeletalAnimationPerObjectComputeRequest->DispatchBoneHIerarchy(a_renderer, *l_boneHierarchyRootSignature, *l_currentFrameResource);
}