...
void FWK::Graphics::StaticModelStandardLitPass::Execute(Renderer & a_renderer, RenderGraph & a_renderGraph)
{
	...
	FWK_ASSERT_RETURN_IF_FAILED(!l_staticModelStandardPerObjectDrawRequest,																		   "StaticModelStandardPerObjectDrawRequestが無効のため、StaticModelStandardLitPassの実行に失敗しました。");
	FWK_ASSERT_RETURN_IF_FAILED(!l_cameraPassDrawRequest->SetupPassConstantBuffer(*l_rootSignature, l_directCommandList, *l_currentFrameResource), "カメラ定数の設定が出来ておらず、StaticModelStandardLitPassの実行に失敗しました。");
	FWK_ASSERT_RETURN_IF_FAILED(!l_lightPassDrawRequest->SetupPassConstantBuffer(*l_rootSignature, l_directCommandList, *l_currentFrameResource),  "ライト定数の設定が出来ておらず、StaticModelStandardLitPassの実行に失敗しました。");
